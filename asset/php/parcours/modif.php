...
// Récupère l'entrée du parcours à modifier
$req = $bdd->prepare('SELECT * FROM mon_parcours WHERE id = ?');
$req->execute([$id]);
$entree = $req->fetch();
...
if (isset($_POST['date'])) {
    ...
    // UPDATE modifie les données. Sans WHERE on modifierait TOUTES les entrées  
    $req = $bdd->prepare('UPDATE mon_parcours SET date=?, info=?, ordre=? WHERE id=?');
    $req->execute([$_POST['date'], $_POST['info'], (int)$_POST['ordre'], $id]);
...
<div class="admin-layout">
  ...
  <div class="page-header"><h1>Modifier</h1></div>

  <form method="POST">
    <div class="form-card">
      <?php // Champ caché : transmet l'id sans l'afficher ?>
      <input type="hidden" name="id" value="<?= $entree['id'] ?>">
      <div class="form-group"><label>Date / Établissement</label><input type="text" name="date" value="<?= $entree['date'] ?>" required></div>
      <div class="form-group"><label>Description</label><textarea name="info" rows="4"><?= $entree['info'] ?></textarea></div>
      <div class="form-group">
        <label>Position (ordre d'affichage)</label>
        <input type="number" name="ordre" value="<?= (int)$entree['ordre'] ?>" min="0">
